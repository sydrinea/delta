...
Deterministic Finite Automata are NFAs with two additional constraints: no epsilon transitions, and exactly one transition per state per symbol. Use `dfa()` from `@delta/build`.

## Basic Structure

```ts
import { dfa } from "@delta/build";
...
`.build()` throws `NFABuildError` (reused from the NFA layer) if there are any errors. In a DFA, missing transitions that would be warnings in an NFA are promoted to errors.
