...
`.machine(nfa)` lets you push a fully built NFA as a sub-expression. This is how larger constructions are assembled from named pieces without re-specifying every state — build each clause separately, then combine them:
