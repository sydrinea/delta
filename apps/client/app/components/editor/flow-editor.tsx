...
import { useSyncFromFlow } from '@/hooks/use-sync-from-flow'
          ...
          {editing
            ? (
                <div
                  ref={containerRef}
                  onBlur={handleBlur}
                  className="flex flex-col gap-1 bg-background border border-muted rounded p-1"
                >
                  <Input
                    autoFocus
                    value={label}
                    ...
                    className="h-6 w-16 border-0 bg-transparent px-1 text-center text-xs text-foreground focus-visible:ring-0"
                  />
                </div>
              )
            : (
                <span
                  ...
                  className="text-xs text-foreground bg-panel/80 px-1.5 py-0.5 rounded cursor-pointer hover:bg-panel-border transition-colors"
                >
                  {label}
                </span>
              )}
  ...
  return (
    <div
      className={`relative w-12 h-12 rounded-full flex items-center justify-center text-sm transition-colors ${data.isAccept
        ? 'border-4 border-double border-foreground bg-panel text-foreground'
        : 'border-2 border-foreground bg-panel text-foreground'
    ...
    >
      <Handle
        ...
        className="bg-primary/45! border-0! w-1! h-1!"
      />
      <Handle
        id="target-left"
        type="source"
...
export function FlowEditor() {
  ...
  return (
    <div className="flex flex-col h-full">
      <div className="flex items-center gap-2 px-4 py-2 border-b border-panel-border shrink-0">
        <Button
          onClick={addState}
          variant="secondary"
          size="xs"
