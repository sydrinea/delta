  ...
  return {
    ...
    cell: (traceStep: TraceStep) => {
      ...
      if (configurations.length === 0)
        return <span className="text-ctp-subtext0 font-mono text-xs">—</span>

      const config = configurations[Math.min(selectedIndex, configurations.length - 1)]!
      if (config.stack.length === 0)
        return <span className="text-ctp-subtext0 font-mono text-xs">ε</span>
      ...
      return (
        <span className="font-mono text-xs text-ctp-subtext1">
          {preview}
          {truncated && <span className="text-ctp-overlay0"> …</span>}
        </span>
      )
    },
  }
    ...
    <div className="flex flex-col gap-3">
      ...
      {configurations.length > 1 && (
        <div className="flex items-center gap-2">
          <span className="text-xs text-ctp-subtext0">Configuration</span>
          <Select
